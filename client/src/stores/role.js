...
export const useRoleStore = defineStore('role', {
  state: () => ({
    ...
    pages: [10, 25, 50, 100],
    valid: false, loading: false, dialog: false, dialogDetail: false,
    headers: () => {
      const { t } = useLocale()
      const h = [
        { title: t('text.name'), key: 'name' },
      ...
      if (store.hasPermissions(['role_update', 'role_delete'])) {
    ...
    model: { role_id: '', name: '', data_status: null, permissions: [] },
    default: { role_id: '', name: '', data_status: null, permissions: [] },
  }),
  actions: {
    async fnSearch() {
    ...
    async fnList({ page = 1, itemsPerPage: limit = 10, sortBy = [] }) {
      ...
      const { data: { items, total } } = await api.get({ url: `/cms/v1/roles`, params })
      this.items = items
      this.total = total
      this.loading = false
    },
    ...
    async fnSave() {
      ...
      if (this.valid) {
        const { success, data } = this.index == -1
          ? await api.post({ url: `/cms/v1/roles`, body: this.model })
          : await api.put({ url: `/cms/v1/roles/${this.model.role_id}`, body: this.model })
        if (success) {
